  ...
  .controller('AppCtrl', function ($scope, $ionicModal, $timeout, $http) {
      ...
      $scope.logout = function () {
        ...
        $scope.checkAuth = function () {
          ...
          var un = $scope.loginCredentials.username;
          var ps = $scope.loginCredentials.password;
          if (localStorage.secret !== '') {
            var secretItems = JSON.parse(localStorage.getItem('secret'));
           // $scope.loginCredentials = secretItems;
            secretItems.uname = un;
            secretItems.keys = ps;
            $http({
              method: 'POST',
              url: $base_url + '/api/user/generate_auth_cookie/?username=' + secretItems.uname + '&password=' + secretItems.keys + '&insecure=cool',
              dataType: "json",
              contentType: "application/json; charset=utf-8"
            }).then(function successCallback(obj) {
              responseText = [obj.data]; // response data 
